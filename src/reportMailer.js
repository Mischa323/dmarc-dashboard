...
async function sendGroupReport(group, db, period) {
  const recipients = db.prepare(
    'SELECT email FROM email_report_recipients WHERE group_id = ?'
  ...
  if (group.transport === 'graph' && group.graph_tenant_id) {
    graphTenant = db.prepare('SELECT * FROM sso_tenants WHERE id = ?').get(group.graph_tenant_id);
  }
  ...
  await sendEmail({
    ...
    smtpConfig: {
      ...
      secure: group.smtp_secure,
      user: group.smtp_user,
      pass: group.smtp_pass,
    },
    graphTenant,
    from: group.smtp_from || (graphTenant && graphTenant.mailbox),
    to: recipients,
    subject,
    html,
  });
...
function _gatherData(group, db, period) {
  ...
  let domainCond = '';
  let domainParams = [];
  if (group.domains_filter && group.domains_filter !== 'all') {
    try {
      const domains = JSON.parse(group.domains_filter);
      if (domains.length > 0) {
...
function _buildHtml(data, period, groupName) {
  ...
  const cell = (label, value, color = '#fff', borderColor = '#1e1e1e') => `
    <td style="padding:0 6px 0 0;">
      <div style="background:#111;border:1px solid ${borderColor};border-radius:10px;padding:14px 16px;text-align:center;min-width:110px;">
...
<div style="max-width:600px;margin:32px auto;padding:0 12px;">
  ...
  <div style="background:#111;border:1px solid #1e1e1e;border-radius:12px;margin-bottom:12px;overflow:hidden;">
    <div style="padding:12px 16px;border-bottom:1px solid #1e1e1e;">
      <b style="font-size:13px;color:#fff;">Domain Breakdown</b>
    </div>
  ...
  <div style="background:#111;border:1px solid rgba(255,69,58,.2);border-radius:12px;margin-bottom:12px;overflow:hidden;">
    <div style="padding:12px 16px;border-bottom:1px solid #1e1e1e;">
      <b style="font-size:13px;color:#ff453a;">⚠ Top Failing Source IPs</b>
    </div>
    <table style="width:100%;border-collapse:collapse;">
      <thead><tr style="background:#0d0d0d;">
        <th style="padding:8px 12px;text-align:left;font-size:10px;color:#555;text-transform:uppercase;">IP Address</th>
        <th style="padding:8px 12px;text-align:right;font-size:10px;color:#555;text-transform:uppercase;">Failed messages</th>
      </tr></thead>
      <tbody>${ipRows}</tbody>
    </table>
  </div>` : ''}
  ...
  <div style="background:#111;border:1px solid rgba(255,159,10,.2);border-radius:12px;margin-bottom:12px;overflow:hidden;">
    <div style="padding:12px 16px;border-bottom:1px solid #1e1e1e;">
      <b style="font-size:13px;color:#ff9f0a;">📮 Reporting Orgs with Failures</b>
    </div>
    <table style="width:100%;border-collapse:collapse;">
      <thead><tr style="background:#0d0d0d;">
  ...
  <p style="text-align:center;font-size:11px;color:#333;margin:20px 0 0;">
    Sent by DMARC Dashboard &middot; Automated report
  </p>
</div>
</body></html>`;
}
